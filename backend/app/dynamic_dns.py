"""
Built-in dynamic DNS via DuckDNS - an optional, opt-in convenience for
networks where mDNS/`.local` resolution doesn't work (some guest Wi-Fi
networks, some routers, and a handful of enterprise setups block it
outright). Purely about reachability - has nothing to do with Google
integration, which authenticates through a service account and never
involves a browser redirect at all, so it doesn't care what hostname or
IP the hub is reachable at.

Instead of walking someone through creating a DuckDNS account, pointing
it at their LAN IP by hand, and remembering to update it if that IP ever
changes, the hub does the update itself - once configured, a background
job (see scheduler.py) keeps it current automatically, the same pattern
already used for Telegram trigger polling.
