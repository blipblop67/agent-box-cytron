"""
Gmail's slice of the generic token lifecycle - just binds the provider name
and which oauth module knows how to refresh. See google_tokens.py for the
actual storage/refresh logic (shared with Drive, Calendar, Sheets).
"""
from . import gmail_oauth, google_tokens
...
def get_valid_access_token(user_id: str) -> str:
    return google_tokens.get_valid_access_token(user_id, PROVIDER, gmail_oauth)
