...
def create_spreadsheet(user_id: str, title: str, headers: list[str] | None = None, sheet_name: str = "Sheet1") -> dict:
    resp = httpx.post(
        API_BASE,
    ...
    if headers:
        update_row_at(user_id, spreadsheet_id, sheet_name, row_number=1, values=headers)
...
def read_rows(user_id: str, spreadsheet_id: str, sheet_name: str = "Sheet1") -> list[list[str]]:
    resp = httpx.get(f"{API_BASE}/{spreadsheet_id}/values/{sheet_name}", headers=_headers(user_id), timeout=30)
    _handle_error(resp, "read")
    return resp.json().get("values", [])


def update_row_at(user_id: str, spreadsheet_id: str, sheet_name: str, row_number: int, values: list[str]) -> None:
...
def append_row(user_id: str, spreadsheet_id: str, sheet_name: str, values: list[str]) -> None:
    resp = httpx.post(
...
def upsert_row(user_id: str, spreadsheet_id: str, sheet_name: str, values: list[str]) -> dict:
    ...
    key = values[0]
    existing_rows = read_rows(user_id, spreadsheet_id, sheet_name)

    for i, row in enumerate(existing_rows):
        if row and row[0] == key:
            row_number = i + 1  # 1-indexed
            update_row_at(user_id, spreadsheet_id, sheet_name, row_number, values)
            return {"action": "updated", "row": row_number, "key": key}

    append_row(user_id, spreadsheet_id, sheet_name, values)
