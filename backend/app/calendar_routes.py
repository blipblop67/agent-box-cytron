"""
Google Calendar integration: connect via OAuth, then list upcoming events
or create new ones. Each team member connects their own calendar - tokens
stored per user_id, encrypted at rest. This is what a "Calendar" tool node
...
from . import calendar_client, calendar_oauth, calendar_tokens, google_oauth, oauth_errors, oauth_state, user_settings
...
def _require_connected(user_id: str):
    if calendar_tokens.get_connection_status(user_id) is None:
        raise HTTPException(400, "Calendar is not connected for this user - call /api/calendar/auth/start first")


@router.get("/events")
def list_events(max_results: int = 10, user: dict = Depends(get_current_user)):
    _require_connected(user["id"])
    return calendar_client.list_events(user["id"], max_results=max_results)
...
def create_event(body: CreateEventRequest, user: dict = Depends(get_current_user)):
    ...
    return calendar_client.create_event(
        user["id"], summary=body.summary, start=body.start, end=body.end,
        description=body.description, location=body.location,
        timezone_name=body.timezone_name, attendees=body.attendees,
    )
