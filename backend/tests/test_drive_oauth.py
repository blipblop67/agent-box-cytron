"""
Proves the "Path B" per-customer Drive OAuth connection flow works end to
end - the same proof as test_gmail_oauth.py, for Drive's own scopes and
routes. See that file for the fuller narrative; this one is intentionally
terser since the pattern is already proven once.
Run with: python3 tests/test_drive_oauth.py
"""
import os
import sys
import tempfile
...
os.environ.setdefault("AGENT_HUB_DATA_DIR", tempfile.mkdtemp(prefix="agent-hub-driveoauth-test-"))
...
def main():
    ...
    assert client.get("/api/drive/auth/start", headers=headers).status_code == 400
    ...
    start_result = client.get("/api/drive/auth/start", headers=headers).json()
    assert "accounts.google.com" in start_result["authorization_url"]
    assert "drive.readonly" in start_result["authorization_url"]
    assert "drive.file" in start_result["authorization_url"]
    real_state = start_result["authorization_url"].split("state=")[1].split("&")[0]
    print("[ok] auth/start returns an authorization URL with Drive's narrower scopes")
    ...
    with patch("httpx.post", side_effect=fake_post), patch("httpx.get", side_effect=fake_get):
        callback_result = client.get(
            "/api/drive/auth/callback", headers=headers,
    ...
    status_after = client.get("/api/drive/status", headers=headers).json()
    ...
    disconnect_result = client.delete("/api/drive/auth", headers=headers).json()
    ...
    print("\nAll Drive OAuth (Path B) smoke tests passed.")
